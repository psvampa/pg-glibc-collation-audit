...
| `test_git_helpers.py` | mostly | the silent-failure class — code that cannot tell "nothing here" from "could not look". Five classes fabricate a tiny glibc-shaped repository instead and run without the clone: the corpus floor, a renamed file gaining an `LC_COLLATE` block, a file gaining one that names no character, and the two about the pair itself — which of the two tags is the newer commit (including a pair that no signal can order, and two probes made to fail) and what step 2 does about it |

...
| `test_git_helpers.py` | mostly | the silent-failure class — code that cannot tell "nothing here" from "could not look". Three classes fabricate a tiny glibc-shaped repository instead and run without the clone: the corpus floor, and a renamed file gaining an `LC_COLLATE` block |

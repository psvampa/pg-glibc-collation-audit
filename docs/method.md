...
- **step 4 over a node's own directory** — `scripts/flag_algorithmic_ranges.py
  ...
  question is asked of a node you supply on its own; step 8 answers it too,
  for both nodes at once, when you supply them both. It was a manual step
  until the seventeenth CHANGELOG entry; a check that depends on somebody
